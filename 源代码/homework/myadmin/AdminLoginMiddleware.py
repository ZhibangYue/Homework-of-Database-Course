...
class AdminLoginMiddleware:
    ...
    def __call__(self, request):
        ...
        urllist = [reverse('myadmin_login'), reverse('myadmin_do_login'), reverse('myadmin_verificode')]

        # 判断是否进入了后台,并且不是进入登录页面
        if re.match('/myadmin/', request.path) and not re.match('/myadmin/google_login',
